...
// ──────────────────────────────────────────────────────────────────────────────
// FrameHeaderV1 — 16 バイト固定長ヘッダ (PROTOCOL_VERSION_V1 = 0x01, v1.0.0)
//
//  Offset | Size | Field
//  -------+------+---------------------------------------------------
//    0    |  4   | magic[4]       {'P','I','P','E'}
//    4    |  1   | version        0x01
//    5    |  1   | flags          FLAG_* ビット（下位 4 ビットのみ有効）
//    6    |  2   | reserved[2]    0x00 0x00
//    8    |  4   | payload_size   ペイロードバイト数（LE）
//   12    |  4   | checksum       CRC-32C（ペイロードのみ対象、LE）
//  -------+------+---------------------------------------------------
//   合計  | 16   |                message_id フィールドなし
// ──────────────────────────────────────────────────────────────────────────────

#pragma pack(push, 1)
struct FrameHeaderV1 {
    uint8_t  magic[4];        // {'P','I','P','E'}
    uint8_t  version;         // PROTOCOL_VERSION_V1 (0x01)
    uint8_t  flags;           // FLAG_* ビット
    uint8_t  reserved[2];     // 送信時は 0x00 で埋めること
    uint32_t payload_size;    // ペイロードサイズ（リトルエンディアン）
    uint32_t checksum;        // CRC-32C（ペイロードのみ、リトルエンディアン）
};
#pragma pack(pop)

static_assert(sizeof(FrameHeaderV1) == 16, "FrameHeaderV1 must be exactly 16 bytes");

// ──────────────────────────────────────────────────────────────────────────────
// FrameHeader — 20 バイト固定長ヘッダ (PROTOCOL_VERSION = 0x02, v1.1.0+)
//
//  Offset | Size | Field
//  -------+------+---------------------------------------------------
//    0    |  4   | magic[4]       {'P','I','P','E'}
...
//    8    |  4   | payload_size   ペイロードバイト数（LE）    ← v1.0.0 と同オフセット
//   12    |  4   | checksum       CRC-32C（ペイロードのみ対象、LE）← v1.0.0 と同オフセット
...
inline constexpr uint8_t  PROTOCOL_VERSION_V1 = 0x01;  // v1.0.0（16B ヘッダ）
inline constexpr uint8_t  PROTOCOL_VERSION    = 0x02;  // v1.1.0+ 現行（20B ヘッダ）

// ─── ヘッダサイズ定数（受信ロジックで使用）──────────────────────────
inline constexpr std::size_t MAGIC_VERSION_SIZE = 5;   // magic[4] + version[1] の先読みサイズ
inline constexpr std::size_t HEADER_SIZE_V1     = sizeof(FrameHeaderV1);  // 16
inline constexpr std::size_t HEADER_SIZE_V2     = sizeof(FrameHeader);    // 20
