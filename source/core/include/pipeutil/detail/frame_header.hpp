...
inline constexpr uint8_t  FLAG_HELLO        = 0x80;  // bit 7: HELLO フレーム (A-001, v1.1.0)
...
// ──────────────────────────────────────────────────────────────────────────────
// HelloPayload — HELLO フレームのペイロード（8 バイト固定）(A-001)
//
//  Offset | Size | Field
//  -------+------+----------------------------------------------------
//    0    |  4   | feature_bitmap  対応 capability の OR (LE)
//    4    |  4   | reserved        0x00000000（将来拡張用）
//  -------+------+----------------------------------------------------
//   合計  |  8   |
// ──────────────────────────────────────────────────────────────────────────────
#pragma pack(push, 1)
struct HelloPayload {
    uint32_t feature_bitmap;  // 対応する capability の OR（リトルエンディアン）
    uint32_t reserved;        // 0x00000000 固定（将来拡張）
};
static_assert(sizeof(HelloPayload) == 8, "HelloPayload must be 8 bytes");

// ──────────────────────────────────────────────────────────────────────────────
// FrameHeaderV1 — v1.0.0 クライアントが送受信する 16 バイトヘッダ (A-001)
//
//  Offset | Size | Field
//  -------+------+---------------------------------------------------
//    0    |  4   | magic[4]       {'P','I','P','E'}
